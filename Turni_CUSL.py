...
def show_loading_screen():
    ...
    loading_window.title("Turni Settimanali")
    loading_window.config(cursor="arrow")
    
    # Add cancelled flag
    loading_window.cancelled = False
    ...
    # Center the window
    window_width = 400
    window_height = 200  # Increased height for spinner
    ...
    # Main frame with normal cursor
    frame = ttk.Frame(loading_window)
    frame.config(cursor="arrow")
    frame.grid(row=0, column=0, sticky='nsew', padx=20, pady=20)
    
    # Configure frame grid
    frame.grid_rowconfigure(1, weight=1)  # Row for canvas
    frame.grid_columnconfigure(0, weight=1)
    
    # Add label
    label = ttk.Label(frame, text="Calcolo dei turni in corso...",
                      font=('Helvetica', 12))
    label.grid(row=0, column=0, pady=10)
    
    # Add spinning circle canvas
    canvas_size = 40
    canvas = tk.Canvas(frame, width=canvas_size, height=canvas_size, highlightthickness=0)
    canvas.grid(row=1, column=0, pady=10)
    
    # Create arc for spinning animation
    arc_start = 0
    loading_window.arc = canvas.create_arc(4, 4, canvas_size-4, canvas_size-4,
                                         start=arc_start, extent=300,
                                         style=tk.ARC, width=4)
    
    def spin_arc():
        if not loading_window.cancelled:
            nonlocal arc_start
            arc_start = (arc_start - 10) % 360  # Changed from + to -
            canvas.itemconfig(loading_window.arc, start=arc_start)
            loading_window.after(50, spin_arc)
    
    # Start animation
    loading_window.after(50, spin_arc)
    
    # Add cancel button with normal cursor
    cancel_button = ttk.Button(frame, text="Annulla", 
                              command=on_cancel,
                              cursor="arrow")
